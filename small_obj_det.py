...
def tiles_location_gen(img_size, tile_size, overlap):
  ...
  # Width and height substract overlap so that we do exceed boundaries in the loops
  # this prevents Dimension Mismatch error from occuring
  for h in range(0, img_height - overlap, h_stride):
    for w in range(0, img_width - overlap, w_stride):
      xmin = w
      ymin = h
      xmax = min(img_width, w + tile_width)   # Note it's taking the minimum here, so the image should not exceed borders
      ymax = min(img_height, h + tile_height)
      yield [xmin, ymin, xmax, ymax]


tile_sizes = "256x256"  # Change this according to your needs
tile_overlap = 128
iou_threshold = 0.1
...
#im_gbgr = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR) # converting it to bgr
...
for i, tile_location in enumerate(tiles_location_gen(img_size, (256, 256), tile_overlap)):  # moving to each tile's location 
    ...
    cv2.imwrite("no_boxes_" + str(i) + ".jpg", tile)
    ...
    for i in range(nms_result[0].shape[0]):
      ...
      plot_one_box(nms_result[0][i][:4], tile, label=labels[int(nms_result[0][i][5])] + " " + str(int(nms_result[0][i][4]*100)) + "%", size=256, line_width=1)
