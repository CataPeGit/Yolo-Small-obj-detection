...
for i, tile_size in enumerate(tile_sizes):  # iterating trough tiles
   for tile_location in tiles_location_gen(img_size, tile_size, tile_overlap):  # moving to each tile's location 
     ...
     for n in range(nms_result[0].shape[0]):
       print("image coordinates are: ", nms_result[0][n][:4])
       plot_one_box(nms_result[0][n][:4], tile, label=labels[int(nms_result[0][n][5])] + " " + str(int(nms_result[0][n][4]*100)) + "%", size=640, line_width=1)
